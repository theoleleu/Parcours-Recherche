...
from senteval.binary import CREval, MREval, MPQAEval, SUBJEval, PREval
...
class SE(object):
    def __init__(self, params, batcher, prepare=None):
        ...
        self.list_tasks = ['CR', 'PR', 'MR', 'MPQA', 'SUBJ', 'SST2', 'SST5', 'TREC', 'MRPC',
    ...
    def eval(self, name, clf):
        ...
        if name == 'CR':
            self.evaluation = CREval(tpath + '/downstream/CR', clf, seed=self.params.seed)
        elif name == 'PR':
            self.evaluation = PREval(tpath + '/downstream/CR', tpath + '/downstream/MR', tpath + '/downstream/MPQA', tpath + '/downstream/SUBJ', clf, seed=self.params.seed)
        elif name == 'MR':
            self.evaluation = MREval(tpath + '/downstream/MR', clf, seed=self.params.seed)
        elif name == 'MPQA':
            self.evaluation = MPQAEval(tpath + '/downstream/MPQA', clf, seed=self.params.seed)
        elif name == 'SUBJ':
            self.evaluation = SUBJEval(tpath + '/downstream/SUBJ', clf, seed=self.params.seed)
        elif name == 'SST2':
        ...
        self.results, clef = self.evaluation.run(self.params, self.batcher)
        #plot(othertask)
        return self.results, clef
