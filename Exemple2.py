...
at1=np.concatenate((np.random.normal(2, sigma, size2), np.random.normal(3.9, sigma, size2), np.random.normal(1, sigma, size2),np.random.normal(5, sigma, size2)))#Le paramètre de la distribution réalise des classes
at2 = np.concatenate((np.random.normal(4, sigma, size2), np.random.normal(1.8, sigma, size2), np.random.normal(6.95, sigma, size2),np.random.normal(5, sigma, size2)))
...
test_loader[0] = [(torch.tensor(i[0]),torch.tensor([i[1]])) for i in zip(TT1,color2)]
